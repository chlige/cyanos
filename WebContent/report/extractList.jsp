...
<% if ( request.isUserInRole(User.SAMPLE_ROLE) ) {
	...
	String sql = "SELECT culture_id, material.material_id, date, extract_info.solvent, extract_info.type, amount_value * POW(10,amount_scale), amount_scale FROM material JOIN extract_info ON(material.material_id = extract_info.material_id) WHERE date >= ? ORDER BY date ASC";
...
<tr><th>Strain</th><th>Extract Date</th><th>Solvent</th><th>Amount</th></tr>
<% while ( results.next() ) {
...
%><%= CyanosObject.autoFormatAmount(volume, BasicObject.MASS_TYPE) %></td></tr>
<% } %>
</table>
<%	results.close();
	sth.close();
