...
if ( request.getParameter("jobid") != null ) { 
	Job job = manager.getJob(request.getParameter("jobid"));
	if ( job != null ) {
%><h1>Job <%= job.getID() %> - <i><%= job.getType() %></i></h1>
<p align="center">
Started: <%= job.getStartDate().toString() %><br>
Ended: <% Date endDate = job.getEndDate(); if ( endDate != null ) {%><%= endDate.toString() %><% } %></p>
<h2>Messages</h2>
<%= job.getMessages() %>
<h2>Output</h2>
<%= job.getOutput() %>
...
<a href="jobs.jsp?jobid=<%= job.getID() %>">Show Results</a>
<script>
