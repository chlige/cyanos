...
<% String contextPath = request.getContextPath();
	...
	Object anObj = request.getAttribute(StrainServlet.STRAIN_OBJECT);
	if ( anObj != null && anObj instanceof DataFileObject ) {
	DataFileObject source = (DataFileObject) anObj;
	String dataFileClass = Strain.DATA_FILE_CLASS;
...
<input type="hidden" name="<%= DataFileServlet.PARAM_DATATYPE %>" value="<%= Strain.PHOTO_DATA_TYPE %>">
<div align="center">
...
<a onclick="updateFileDiv('<%= div %>', '<%= previous %>', '<%= dataFileClass %>', '<%= Strain.PHOTO_DATA_TYPE %>')"><i>Parent directory</i></a><% } %></td></tr>
...
<ul><% Arrays.sort(kids, DataFileServlet.directoryFirstCompare());
	...
	ExternalFile myFiles = source.getDataFilesForType(Strain.PHOTO_DATA_TYPE);					
...
%><li><a onclick="updateFileDiv('<%= div %>', '<%= path.concat(dirName) %>', '<%= dataFileClass %>', '<%= Strain.PHOTO_DATA_TYPE %>')"><%= dirName %></a></li><% } else { break; } i++; } %>
</ul>
	...
	while ( i < kids.length ) {
		...
		if ( update ) {
			if ( myDesc != null && (! addedPaths.containsKey(thisPath)) ) {
				...
				source.linkDataFile(thisPath, Strain.PHOTO_DATA_TYPE, myDesc, null);
			}
		}
...
<div align="center">
<!--  
<iframe src="<%= contextPath %>/file/upload?<%= String.format("%s=%s", DataFileServlet.DATAFILE_CLASS, Strain.DATA_FILE_CLASS) %>&<%= String.format("%s=%s", DataFileServlet.PARAM_DATATYPE, Strain.PHOTO_DATA_TYPE) %>" 
