...
<th class="header" width='100'><a class="sort" onClick='sortTable("<%=divID%>","<%= Taxon.LEVEL_ORDER  %>","<%=( sortField.equals(Taxon.LEVEL_ORDER) && sortAsc ? SQLStrain.DESCENDING_SORT : SQLStrain.ASCENDING_SORT)%>")'>Order</a>
<%	if ( sortField.equals(Taxon.LEVEL_ORDER) ) { %><img src="<%=contextPath%>/images/<%=(sortAsc ? "sort-asc.png" : "sort-desc.png")%>"><% } %></th>
...
<td><c:out value="<%= queryResults.getTaxonName(Taxon.LEVEL_ORDER) %>"/></td>
